...
const PreviewSalesSOL = require('sol-preview-sales')
...
BotSOL.prototype.closePopup = async function() {
  ...
  try {
    const popup = await new Promise((resolve, reject) => {
      this.page.on('campaign-data', (data) => {
        resolve(data)
...
BotSOL.prototype.getInformation = async function() {
  const objPreviewSales = new PreviewSalesSOL(this.period, this.page)

  try {
    await objPreviewSales.goto()

    return await objPreviewSales.getInformation()
  } catch (e) {
    throw e
  }
}
